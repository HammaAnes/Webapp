...
export const WIZARD_STEPS: WizardStepConfig[] = [
  ...
  { id: 7, title: 'Récapitulatif', icon: 'CheckCircle' },
];
...
export function useDomiciliationWizard(
  user: User,
  ...
  const canProceed = useMemo(() => {
    switch (currentStep) {
      ...
      case 3: {
        ...
        return !!(d.adresseResidence?.trim() && d.ville?.trim() && d.email?.trim() && formData.dateDebutSouhaitee);
      }
