...
import { useCourrier } from '../../hooks/useDomiciliation';
import type { DomiciliationOptions, CourrierItem } from '../../domain/types';
...
function CourrierCard({ courrier, options, onAction }: {
  ...
  const handleInstructions = async () => {
    ...
    try {
      await apiClient.donnerInstructionCourrier(courrier.id, instructionText);
            ...
            <div className="mt-3 space-y-2">
              <div className="flex flex-wrap gap-2">
                <Button
                  size="sm"
                  variant="primary"
                  loading={actionLoading === 'recuperer'}
                  onClick={() => onAction(courrier.id, 'recuperer', 'Demande de récupération')}
                >
                ...
                {canScan && (
                  <Button
                    ...
                    onClick={() => onAction(courrier.id, 'scanner', 'Demande de numérisation')}
                  >
                ...
                {canReexpedition && (
                  <Button
                    size="sm"
                    variant="outline"
                    loading={actionLoading === 'reexpedier'}
                    onClick={() => onAction(courrier.id, 'reexpedier', 'Demande de réexpédition')}
                  >
              ...
              <div className="flex gap-2 items-center">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  loading={actionLoading === 'instruction'}
...
export default function CourrierUtilisateur({ domiciliationId, options }: CourrierUtilisateurProps) {
  const { courriers, loading, reload } = useCourrier(domiciliationId);
      ...
      {paginated.length === 0 ? (
        ...
        <div className="space-y-3">
          {paginated.map(courrier => (
            <CourrierCard key={courrier.id} courrier={courrier} options={options} onAction={handleAction} />
          ))}
        </div>
      )}
