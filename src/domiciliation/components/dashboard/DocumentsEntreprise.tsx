...
import type { DocumentRecord, DocumentSlot, TypeStructure } from '../../domain/types';
import { documentFromAPI } from '../../adapters/apiAdapter';
...
async function loadDocumentsFromApi(entityId: string): Promise<DocumentRecord[]> {
  const response = await apiClient.getDocuments('domiciliation', entityId);
  ...
  return raw.map(documentFromAPI);
}

async function triggerDownload(doc: DocumentRecord): Promise<void> {
...
async function getPreviewUrl(doc: DocumentRecord): Promise<string | null> {
...
const ActionBtn: React.FC<ActionBtnProps> = ({ onClick, icon: Icon, label, cls }) => (
  <button
    onClick={onClick}
    className={`text-xs flex items-center gap-1 px-2 py-1 rounded-md transition-colors ${cls}`}
  >
...
export default function DocumentsEntreprise({ domiciliationId, typeStructure, readOnly = false }: DocumentsEntrepriseProps) {
  ...
  const allSlots: DocumentSlot[] = getDocumentSlots(typeStructure);
  const getDoc = (type: string) => documents.find((d) => d.documentType === type);

  const loadDocs = useCallback(async () => {
    try {
      setLoading(true);
      const docs = await loadDocumentsFromApi(domiciliationId);
      setDocuments(docs);
    } catch {
      setDocuments([]);
    } finally {
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500">{uploadedRequired} sur {requiredSlots.length} documents requis</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {allSlots.map((slot) => {
          ...
          return (
            <Card
              key={slot.type}
              className={`p-4 transition-all ${doc
              ...
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${doc ? 'bg-emerald-100' : slot.required ? 'bg-amber-100' : 'bg-gray-100'}`}>
                  {doc
                    ...
                    : <FileText className={`w-5 h-5 ${slot.required ? 'text-amber-600' : 'text-gray-400'}`} />
                  }
                </div>
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Supprimer le document">
        <div className="space-y-4">
          <p className="text-gray-700">
            Êtes-vous sûr de vouloir supprimer le document <strong>{deleteTarget?.fileName}</strong> ?
          </p>
