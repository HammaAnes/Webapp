...
import { format, isToday, isTomorrow, differenceInMinutes, isAfter, isBefore, addMinutes } from 'date-fns';
...
export default function Aujourdhui() {
  const { reservations, espaces, initializeData, loadUsers } = useAppStore();
  ...
  const stats = useMemo(() => {
    ...
    const cancelledToday = todayReservations.filter(r => r.statut === 'annulee');
  ...
  const ReservationCard = ({ reservation, showBadge = true }: { reservation: Reservation; showBadge?: boolean }) => {
    ...
    return (
      <Card className={`p-4 hover:shadow-md transition-all ${isActive ? 'border-l-4 border-emerald-500' : ''}`}>
            ...
            <div className="space-y-1.5 text-sm text-gray-600">
              ...
              {reservation.utilisateur?.telephone && (
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href={`tel:${reservation.utilisateur.telephone}`}
